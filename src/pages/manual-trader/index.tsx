...
const digitOf = (quote: number, pipSize?: number) => {
  let decimals = 2;
  if (typeof pipSize === 'number' && pipSize > 0 && pipSize < 1) {
    decimals = Math.max(0, Math.round(-Math.log10(pipSize)));
  }
...
const ManualTrader = () => {
    ...
    ws.onmessage = event => {
      try {
        ...
        if (data.msg_type === 'history' && Array.isArray(data.history?.prices)) {
          ...
          setDigits(list);
          if (data.history.prices.length) setQuote(String(data.history.prices[data.history.prices.length - 1]));
        }
        ...
        // Ignore malformed public feed packets.
      }
    };
  ...
  const latest = digits.length ? digits[digits.length - 1] : undefined;
  ...
  const card: React.CSSProperties = {
    background: 'var(--general-section-1,#fff)',
    border: '1px solid var(--general-section-2,#d9dee7)',
    borderRadius: 14,
    padding: 14,
  };
  const input: React.CSSProperties = {
    ...
    background: 'var(--general-section-1,#fff)',
    color: 'var(--text-prominent,#172033)',
  };

  return (
    <main style={{ minHeight: '100%', padding: '14px 12px 100px', background: 'var(--general-main-1,#fff)', color: 'var(--text-prominent,#172033)' }}>
      <div style={{ maxWidth: 1120, margin: '0 auto' }}>
        <section style={{ ...card, display: 'flex', gap: 10, alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap' }}>
          <div>
            <small style={{ opacity: 0.6, fontWeight: 800 }}>MANUAL TRADING • DIGITS</small>
            <h1 style={{ margin: '5px 0 2px' }}>Manual Trader</h1>
            <span style={{ color: live ? '#159957' : '#c0392b', fontWeight: 800 }}>● {live ? 'LIVE TICK STREAM' : 'CONNECTING'}</span>
          </div>
        ...
        <section style={{ ...card, marginTop: 12 }}>
          ...
          <div style={{ marginTop: 10, textAlign: 'center' }}><small style={{ opacity: 0.6 }}>CURRENT TICK</small><div style={{ fontSize: 28, fontWeight: 900, color: '#43b5d8' }}>{quote}</div></div>

          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5,minmax(0,1fr))', gap: 9, marginTop: 10 }}>
            {Array.from({ length: 10 }, (_, d) => (
              <div key={d} style={{ textAlign: 'center' }}>
                <div style={{ width: 58, height: 58, maxWidth: '100%', margin: '0 auto', borderRadius: '50%', display: 'grid', placeItems: 'center', background: d === latest ? '#1fbd84' : '#f1f3f5', color: d === latest ? '#fff' : '#222', border: d === latest ? '3px solid #0a9b67' : '1px solid #e0e4e8', fontWeight: 900, fontSize: 19, position: 'relative', boxShadow: d === latest ? '0 0 0 6px rgba(31,189,132,.12)' : undefined }}>
                  {d}{d === latest && <span style={{ position: 'absolute', bottom: -20, left: '50%', transform: 'translateX(-50%)', fontSize: 18, color: '#e44' }}>▲</span>}
                </div>
                <small style={{ fontWeight: 800 }}>{digits.length ? ((counts[d] / digits.length) * 100).toFixed(1) : '0.0'}%</small>
              </div>
            ))}
          </div>
